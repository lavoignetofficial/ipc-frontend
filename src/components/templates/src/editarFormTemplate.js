...
export const EDITARFORM = props => {
    ...
    const { uploadFilesAdjunt, getImages } = filesContext;
    ...
    const { modificarForm, formActual } = formContext;
    ...
    const [fields, saveField] = useState({
        fileImages: formActual.archivos ? formActual.archivos.attachment : null,
        numeroCliente: formActual.cliente,
        numeroInspector: formActual.inspector,
        formsConstruction: formActual.construccion,
        formsFloors: formActual.pisos,
        formsTypeInm: formActual.tipoInmueble,
        items: formActual.items,
        tipoInspeccion: formActual.tipoInspeccion,
    });
    ...
    const onSubmit = async e =>{
        ...
        if(fields.attachment){
            files = await uploadFilesAdjunt(fields.attachment.formData)
        }
        
        const Form = await modificarForm(
            formActual._id,
        {
            pisos: fields.formsFloors,
            archivos: {
                attachment: files ? files.data.name : null
            },
    ...
    const submitFile = file => {
        console.log(fields["formsAdjuntfiles"])
    }
        ...
        <Container className="card-template">
            ...
            <Card>
                <Card.Body>
                    ...
                    <Row>
                        <Item zonas={zonas} actionFields={{fields: fields, saveField: saveField}} elementos={formActual.items}/>
                    </Row>
                    <Row>
            ...
            <Row className="space-display-top">
                <Col xs={2}></Col>
                <Col xs={4} className="center">
                    <Button className="color-principal space-display-right" type="button" onClick={onSubmit}>Guardar Formulario</Button>
                </Col>
                <Col xs={4} className="center">
                    <Button className="color-principal space-display-left" type="button">Imprimir</Button>
                </Col>
                <Col xs={2}></Col>
            </Row>
        </Container>)
